...
public class MetalWallUniverse : MonoBehaviour
{
    ...
    float xSpeed = 2.0f; //how fast it moves
    float yTranslate = 0.05f;
    float amount = 0.01f; //how much it moves
    ...
    public void MoveBalloon(GameObject balloon, int index)
    {
        ...
        if (balloon != null)
        {
            ...
            {
                p.x = balloon.transform.position.x + Mathf.Sin(Time.time * xSpeed) * amount;
                p.y = balloon.transform.position.y + yTranslate;
